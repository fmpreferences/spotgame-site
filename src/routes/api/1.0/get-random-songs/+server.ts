...
export const POST: RequestHandler = async ({ request, platform }) => {
    ...
    console.log(`select distinct a.title, a.id, artist_name, album_art, artist_index from tracks a join track_artists b on a.id = b.id join artists c on b.artist_id = c.artist_id
    ...
    const rows = await platform?.env.DB.prepare(
        `select distinct a.title, a.id, artist_name, album_art, artist_index from tracks a join track_artists b on a.id = b.id join artists c on b.artist_id = c.artist_id
        where a.id in (select d.id from tracks d join buckets e on d.id = e.id where date between '${min_year}-01-01' and '${max_year}-12-31' and 
        streams >= ${min_streams} and bucket in (${active.map((e: any) => `'${e.genre}'`)}) order by random() limit ${n})`
    ).run();
